inp = """febca cfagb ecbafd efdcbg cbegdfa fg bgafec gfae acgdb gfc | cgf facdeb ecgfdb afcbge
...
for line in inp:
    a, b = line.split(" | ")
    a = a.split(" ")
    ones = set([abc for abc in a if len(abc) == 2][0])
    fours = set([abc for abc in a if len(abc) == 4][0])
    sevens = set([abc for abc in a if len(abc) == 3][0])
    eights = set([abc for abc in a if len(abc) == 7][0])
    nines = set([abc for abc in a if fours.issubset(set(abc)) and len(abc) == 6][0])
    sixes = set([abc for abc in a if len(abc) == 6 and not set(abc).issubset(nines) and not ones.issubset(set(abc))][0])
    zeros = set([abc for abc in a if len(abc) == 6 and not set(abc).issubset(nines) and ones.issubset(set(abc))][0])
    threes = set([abc for abc in a if len(abc) == 5 and sevens.issubset(set(abc))][0])
    fives = set([abc for abc in a if len(abc) == 5 and len(set(abc).intersection(fours)) == 3 and set(abc) != threes][0])
    twos = set([abc for abc in a if len(abc) == 5 and len(set(abc).intersection(fours)) == 2][0])
    numbers = [zeros, ones, twos, threes, fours, fives, sixes, sevens, eights, nines]
    right = [set(n) for n in b.split(" ")]
    newright = [0]*4
    for i in range(10):
        for j in range(4):
            if set(right[j]).issubset(numbers[i]) and numbers[i].issubset(set(right[j])):
                newright[j] = i
    ans += int("".join([str(a) for a in newright]))
    t = int("".join([str(a) for a in newright]))
    # print(t)
print(ans)