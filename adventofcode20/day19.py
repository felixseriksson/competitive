...
inp = inp.split("\n")
rules = inp[:inp.index("")]
message = inp[inp.index("")+1:]

rulesdict = dict()
for rule in rules:
    name, contents = rule.split(": ")
    contents = contents.split(" | ")
    # rulesdict[int(name)] = []
    rulesdict[name] = []
    for k in contents:
        if " " in k:
            #rulesdict[int(name)].append([int(char) for char in k.split(" ")])
            rulesdict[name].append(k.split(" "))
        else:
            #rulesdict[int(name)].append(k[1])
            rulesdict[name].append(k[1])

# for key, value in rulesdict.items():
#     print(key)
#     print(value)

match = "".join([char for char in rulesdict["0"][0]])
while not set(match).issubset(set(["a", "b", "|", "(", ")"])):
    newmatch = ""
    for char in match:
        if not char.isnumeric():
            newmatch += char
            continue
        contents = rulesdict[char]
        if len(contents) == 1:
            newmatch += "".join([char for char in contents[0]])
        else:
            tmp = "("
            for c in contents:
                tmp += "".join([char for char in c])
                tmp += "|"
            tmp = tmp[:-1]
            tmp += ")"
            newmatch += tmp
