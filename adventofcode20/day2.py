...
inp = [([int(x) for x in newpart[0].split("-")], newpart[1][:-1], newpart[2]) for newpart in [part.split() for part in inp.split("\n")]]
ctr = 0
for row in inp:
    # policy 1
    # ct = row[2].count(row[1])
    # if ct >= row[0][0] and ct <= row[0][1]:
    #     ctr += 1
          # svar: 645
    # policy 2
    if (row[2][row[0][0] - 1] == row[1] and row[2][row[0][1] - 1] != row[1]) or (row[2][row[0][1] - 1] == row[1] and row[2][row[0][0] - 1] != row[1]):
        ctr += 1
        # svar: 737
print(ctr)