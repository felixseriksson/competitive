import re
inp = """iyr:2015 cid:189 ecl:oth byr:1947 hcl:#6c4ab1 eyr:2026
hgt:174cm
pid:526744288
...
# # part 1
# for i in inp:
#     keys = set([k.split(":")[0] for k in i])
#     if "cid" in keys and len(keys) == 8:
#         ctr += 1
#     elif not "cid" in keys and len(keys) == 7:
#         ctr += 1
#     else:
#         pass
# print(ctr) # output: 264
# part 2
for i in inp:
    tmp = [(k.split(":")[0],k.split(":")[1]) for k in i]
    dic = {a:b for a,b in tmp}
    keys = set(dic.keys())
    if "byr" in keys:
        if len(dic["byr"]) == 4 and int(dic["byr"]) >= 1920 and int(dic["byr"]) <= 2002:
            if "iyr" in keys:
                if len(dic["iyr"]) == 4 and int(dic["iyr"]) >= 2010 and int(dic["iyr"]) <= 2020:
                    if "eyr" in keys:
                        if len(dic["eyr"]) == 4 and int(dic["eyr"]) >= 2020 and int(dic["eyr"]) <= 2030:
                            if "hgt" in keys:
                                unit = dic["hgt"][-2:]
                                if (unit == "cm" and int(dic["hgt"][:-2]) >= 150 and int(dic["hgt"][:-2]) <= 193) or (unit == "in" and int(dic["hgt"][:-2]) >= 59 and int(dic["hgt"][:-2]) <= 76):
                                    if "hcl" in keys:
                                        if dic["hcl"][0] == "#" and len(dic["hcl"][1:]) == 6 and dic["hcl"][1:].isalnum():
                                            if "ecl" in keys:
                                                if dic["ecl"] in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
                                                    if "pid" in keys:
                                                        if len(dic["pid"]) == 9 and dic["pid"].isnumeric():
                                                            ctr += 1
print(ctr) #output: 224